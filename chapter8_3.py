...
;  его код в таблице ASCII, и функция chr(), которая по коду символа
;   возвращает сам символ. Например, ord('a') == 97, chr(97) == 'a'.



def capitalize(word):
    return chr(ord(word[0]) + ord('A') - ord('a')) + word[1:]
a = [str(a) for a in input().split()] #массив стал строкой
